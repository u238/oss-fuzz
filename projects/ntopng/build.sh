...
for f in $(find $SRC -name '*_fuzzer.cpp'); do
    ...
    $CXX -std=c++0x -O1 -fno-omit-frame-pointer -gline-tables-only $CXXFLAGS $INCLUDE_DIRS -stdlib=libc++ -c $f -o src/$b.o
    $CXX -std=c++0x -O1 -fno-omit-frame-pointer -gline-tables-only $CXXFLAGS $INCLUDE_DIRS $O_FILES src/$b.o -o $OUT/$b $LIB_FUZZING_ENGINE -Wall $LIBS -Wl,-rpath,'$ORIGIN/lib'
